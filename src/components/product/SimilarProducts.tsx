...
export default async function SimilarProducts({
  ...
  try {
    ...
    const rawSimilarProducts =
      await ProductApi.getSimilarProducts(categorySlug);

    // 3. Filter out the current product so it doesn't show in its own "Similar" list
    const similarProducts = rawSimilarProducts.filter(
      (p) => p.id !== productId,
    );
      ...
      <section className="space-y-6 pt-4">
        ...
        <div className="flex overflow-x-auto gap-4 pb-4 no-scrollbar snap-x">
          {similarProducts.map((item) => {
            ...
            return (
              <Link
                href={`/product/${item.slug}`} // Or item.slug if your similar schema has it
                key={item.id}
                ...
                <div className="flex flex-col flex-1">
                  ...
                  <div className="mt-auto pt-2 flex flex-col">
                    <div className="flex items-center space-x-1">
                      ...
                      {/* Note: SimilarProductSchema didn't have rating, if you add it, use dynamic stars here too */}
                    </div>
